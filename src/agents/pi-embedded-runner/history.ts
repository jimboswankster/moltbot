...
export function getSessionHistoryLimitFromSessionKey(
  sessionKey: string | undefined,
  config: OpenClawConfig | undefined,
): number | undefined {
  if (!sessionKey || !config) {
  ...
  const kind = providerParts[1]?.toLowerCase();
  if (kind === "dm") {
    return getDmHistoryLimitFromSessionKey(sessionKey, config);
  }
  if (kind !== "group") {
    return undefined;
  }
  ...
  return (entry as { historyLimit?: number }).historyLimit;
}

...
export function createSessionsSendTool(opts?: {
  ...
  return {
    ...
    execute: async (_toolCallId, args) => {
      ...
      log.debug("sessions_send execute start", {
        toolCallId,
        agentSessionKey: opts?.agentSessionKey ?? "(not set)",
        agentChannel: opts?.agentChannel ?? "(not set)",
      ...
      try {
        ...
        if (timeoutSeconds === 0) {
          log.debug("sessions_send (async mode) calling gateway agent method", {
            toolCallId,
            targetSessionKey: resolvedKey,
            displayKey,
            requesterSessionKey: opts?.agentSessionKey,
          ...
          try {
            ...
            log.debug("sessions_send (async mode) agent method returned", {
              toolCallId,
              runId,
            ...
            log.error("sessions_send (async mode) agent method failed", {
              toolCallId,
              error: messageText,
              targetSessionKey: resolvedKey,
            });
        ...
        log.debug("sessions_send calling gateway agent method", {
          toolCallId,
          targetSessionKey: resolvedKey,
          displayKey,
          requesterSessionKey: opts?.agentSessionKey,
          isCrossAgent,
          lane: sendParams.lane,
        ...
        try {
          ...
          log.debug("sessions_send agent method returned", {
            toolCallId,
            runId,
            hadRunIdResponse: !!response?.runId,
          });
          ...
          const messageText =
            err instanceof Error ? err.message : typeof err === "string" ? err : "error";
          log.error("sessions_send agent method failed", {
            toolCallId,
            error: messageText,
            targetSessionKey: resolvedKey,
          });
        ...
        log.debug("sessions_send calling agent.wait", {
          toolCallId,
          runId,
          timeoutMs,
        });
        try {
          ...
          log.debug("sessions_send agent.wait returned", {
            toolCallId,
            runId,
            waitStatus,
            waitError: waitError ?? "(none)",
          });
        } catch (err) {
          const messageText =
            err instanceof Error ? err.message : typeof err === "string" ? err : "error";
          log.error("sessions_send agent.wait failed", {
            toolCallId,
            runId,
            error: messageText,
          });
        ...
        // Ensure any uncaught error returns a proper tool result instead of "No result provided"
        const errorMessage = err instanceof Error ? err.message : String(err);
        log.error("sessions_send unexpected error", {
          toolCallId,
          error: errorMessage,
          agentSessionKey: opts?.agentSessionKey ?? "(not set)",
        });
        return jsonResult({
          runId: crypto.randomUUID(),
          status: "error",
          error: `sessions_send failed: ${errorMessage}`,
        });
      }
    },
  };

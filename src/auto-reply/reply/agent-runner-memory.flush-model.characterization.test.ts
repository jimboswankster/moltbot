/**
 * CHARACTERIZATION TESTS — H-2/H-6: Memory Flush Model + Fallbacks
 *
 ...
 * Broken behavior: flush inherits followup run model and passes agent-scoped fallbacks
 * Paired remediation contract: agent-runner-memory.flush-model.contract.test.ts
 * Lifecycle: DELETE after fix H-2/H-6 commits
 *
...
describe("runMemoryFlushIfNeeded model selection characterization", () => {
  it("passes followup run provider/model + agent fallbacks to runWithModelFallback", async () => {
    ...
    runWithModelFallbackMock.mockImplementation(
      async ({ provider, model, run, fallbacksOverride }) => {
        await run(provider, model);
        return { provider, model, fallbacksOverride };
      },
    );
    ...
    expect(call.fallbacksOverride).toEqual(["openai/gpt-4o-mini"]);
  });
});
